...
var plugins = [
  ...
  new webpack.DefinePlugin({
        'ENV': {
            'environment': JSON.stringify(process.env.API),
        }
  }),
  //自动分析重用的模块并且打包成单独的文件
  ...
  function(){
      return this.plugin('done', function(stats) {
        ...
        console.log('版本是：'+JSON.stringify(stats.toJson().hash));//拿到hash值
...
module.exports = {
    ...
    output: {
         ...
         */
        publicPath: "/dist/",
        filename:production ? "build.[hash].js" : "build.js"//[hash]MD5戳   解决html的资源的定位可以使用 webpack提供的HtmlWebpackPlugin插件来解决这个问题  见：http://segmentfault.com/a/1190000003499526 资源路径切换
    },
    module: {
        ...
        loaders: [
            ...
            {test: /\.(png|jpg)$/, loader: 'url-loader?limit=10240'} // 内联 base64 URLs, 限定 <=10k 的图片, 其他的用 URL
